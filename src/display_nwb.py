...
class ReadNWB():
    ...
    def readfile(self, f: Union[Path, str, None]=None):
        ...
        if recmode in ['CC', 'IC']:  # get command and response data .. 
            self.command = nwbfile.acquisition['Ics1'].data[:]
            self.response = nwbfile.acquisition['Vcs1'].data[:]
            try:
                self.opto = nwbfile.acquisition['ChR2 Stimulation 1']
            except:
                self.opto = None
        elif recmode == 'VC':
            self.response = nwbfile.acquisition['Ics1'].data[:]
            self.command = nwbfile.acquisition['Vcs1'].data[:]
            try:
                self.opto = nwbfile.acquisition['ChR2 Stimulation 1']
            except:
                self.opto = None
    ...
    def plot_traces(self, recmode:str):
        ...
        for i in range(self.response.shape[1]):
            axs['A'].plot(self.time, self.response[:,i]+i*100e-12)
            axs['B'].plot(self.time, self.command[:,i])

            if self.opto is not None:
                axs['C'].plot(self.optotimebase, self.opto.data[:, i])
        axs['F'].plot(self.time, np.mean(self.response, axis=1))
        self.trace_plot = axs['E'].plot(self.time, self.response[:,0], 'g-')
